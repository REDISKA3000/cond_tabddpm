...
def ohe_to_categories(ohe, K):
    ...
    indices = torch.cat(
        [torch.zeros((1,)), K.cumsum(dim=0)], dim=0).int().tolist()
    res = []
...
def index_to_log_onehot(x, num_classes):
    ...
    for i in range(len(num_classes)):
        # print('num classes i', num_classes[i], len(np.unique(x[:,i].to('cpu'))))
        # display(F.one_hot(x[:, i], num_classes[i]))
...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        alphas_cumprod_prev = torch.tensor(
            np.append(1.0, alphas_cumprod[:-1]))  # tilde_alpha2_{t-1}
        alphas_cumprod_next = torch.tensor(
            np.append(alphas_cumprod[1:], 0.0))  # tilde_alpha2_{t+1}
        sqrt_alphas_cumprod = np.sqrt(alphas_cumprod)    # tilde_alpha_t
        sqrt_one_minus_alphas_cumprod = np.sqrt(
            1.0 - alphas_cumprod)  # tilde_beta_t
        sqrt_recip_alphas_cumprod = np.sqrt(
            1.0 / alphas_cumprod)    # sqrt(1 / tilde_alpha_t)
        ...
        self.register_buffer(
            'alphas_cumprod', alphas_cumprod.float().to(device))
    ...
    def gaussian_p_mean_variance(
        ...
        model_variance = torch.cat([self.posterior_variance[1].unsqueeze(
            0).to(x.device), (1. - self.alphas)[1:]], dim=0)
    ...
    def q_posterior(self, log_x_start, log_x_t, t):
        ...
        t_broadcast = t.to(log_x_start.device).view(-1, *
                                                    num_axes) * torch.ones_like(log_x_start)
        ...
        unnormed_logprobs = log_EV_qxtmin_x0 + \
            self.q_pred_one_timestep(log_x_t, t)
    ...
    def kl_prior(self, log_x_start):
        ...
        log_half_prob = - \
            torch.log(self.num_classes_expanded *
                      torch.ones_like(log_qxT_prob))
    ...
    def sample_time(self, b, device, method='uniform'):
        if method == 'importance':
            ...
            t = torch.multinomial(pt_all, num_samples=b,
    ...
    def loss_at_step_t(self, x, step):
        ...
        if x_cat.shape[1] > 0:
            loss_multi = self._multinomial_loss(
                model_out_cat, log_x_cat, log_x_cat_t, t, pt) / len(self.num_classes)
        ...
        recon_loss = self._gaussian_loss(
            recon_x0_num, x_num, x_num_t, t, x_num)
    ...
    def recon_x0(self, x, model_out, t):
        ...
        x0 = extract(self.sqrt_recip_alphas_cumprod, t, x.shape) * (x -
                                                                    model_out * extract(self.sqrt_one_minus_alphas_cumprod, t, x.shape))

        return x0

    def mixed_loss(self, x, x_cond=None):
        # print('x', x.shape)
        # print(6, x.shape[1] - 6, x_cond.shape)
        # print('rhehhehrhrhrhrhrhr')
        b = x.shape[0]
        device = x.device
        t, pt = self.sample_time(b, device, 'uniform')

        # print('t pt', t, pt)
        # print(t.shape, pt.shape)
        ...
        log_x_cond = x_cond
        # print('x cond:')
        # display(pd.DataFrame(x_cond))
        # display(pd.DataFrame(x_cond).value_counts())
        x_in = 0
        if x_num.shape[1] > 0:
            noise = torch.randn_like(x_num)
            x_num_t = self.gaussian_q_sample(x_num, t, noise=noise)
        if x_cat.shape[1] > 0:
            log_x_cat = index_to_log_onehot(x_cat.long(), self.num_classes)
            # print('log x cat log x cat log x cat log x cat log x cat log x cat log x cat log x cat log x cat log x cat log x cat ')
            # display(log_x_cat)
            log_x_cat_t = self.q_sample(log_x_start=log_x_cat, t=t)
        if x_cond.shape[1] > 0:
            # print('x_cond shape', x_cond.shape, len(np.unique(x_cond.to('cpu'))))
            # print('cond_features',self.cond_features)
            log_x_cond = index_to_log_onehot(x_cond.long(), self.cond_features)
            # log_x_cond = log_x_cond / (-69.0776) + 0
            # print('log x cond log x cond log x cond log x cond log x cond log x cond log x cond log x cond log x cond ')
            # display(log_x_cond)
            # print(log_x_cond >= 0)
        # if x_cond is not None:
          # x_in = torch.cat([x_num_t, log_x_cat_t, log_x_cond], dim=1)
        # else:
        x_non_cond = torch.cat([x_num_t, log_x_cat_t], dim=1)

        log_x_cat = index_to_log_onehot(x_cat.long(), self.num_classes)

        # print('AHAHHAHAHHAHAHHAHAHHAHAHHAHAHHAHAHHAHAHAHAHHAHHA')
        # print('log x cat')
        # display(log_x_cat)
        # display(log_x_cat_t)
        ...
        # print('x cond long')
        # display(x_cond.long())
        # display(F.one_hot(x_cond.long(), 2) / (-69.0776))

        # display(log_x_cond / (-69.0776) + 0)

        x_non_cond = x_non_cond.to(device)
        log_x_cond = log_x_cond.to(device)
        model_out = self._denoise_fn(
            x_non_cond=x_non_cond,
            x_cond=x_cond,
            timesteps=t
        )
        # print('x in', x_in.shape)
        # print(x_num.shape, log_x_cat.shape, log_x_cond.shape)

        model_out_num = model_out[:, :self.num_numerical_features]
        model_out_cat = model_out[:,
                                  self.num_numerical_features:model_out.shape[1]]

        loss_multi = torch.zeros((1,)).float()
        loss_gauss = torch.zeros((1,)).float()
        # print('model out',model_out.shape)
        # print('model out cat', model_out_cat.shape, model_out_num.shape)
        # print(self.num_classes, len(self.num_classes))
        # print('num_classes',len(self.num_classes), model_out_cat.shape, log_x_cat_t.shape, log_x_cat.shape)
    ...
    def mixed_elbo(self, x0):
        ...
        if has_cat:
            log_x_cat = index_to_log_onehot(
                x_cat.long(), self.num_classes).to(device)
        ...
        for t in range(self.num_timesteps):
            ...
            x_num_t = self.gaussian_q_sample(
                x_start=x_num, t=t_array, noise=noise)
            if has_cat:
            ...
            eps = self._predict_eps_from_xstart(
                x_num_t, t_array, out["pred_xstart"])
    ...
    def multinomial_ddim_step(
        ...
        if t[0] != 0:
            alpha_bar_prev = extract(
                self.alphas_cumprod, t_prev, log_x_t.shape)
        else:
            alpha_bar_prev = extract(
                self.alphas_cumprod_prev, t_prev, log_x_t.shape)
    ...
    @torch.no_grad()
    def sample(self, num_samples, log_x_cond=[]):
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn((b, self.num_numerical_features), device=device)
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            log_x_cond = log_x_cond.to(self.device)
            x_in = torch.cat([z_norm, log_z, log_x_cond], dim=1).float()
            ...
            z_norm = self.gaussian_p_sample(
                model_out_num, z_norm, t, clip_denoised=False)['sample']
            if has_cat:
    ...
    def sample_all(self, num_samples, batch_size, ddim=False, steps=1000, x_cond=[]):
        if ddim:
        ...
        if x_cond.shape[1] > 0:
            log_x_cond = index_to_log_onehot(x_cond.long(), self.cond_features)
            log_x_cond = log_x_cond / (-69.0776) + 0
        log_x_cond_batch = log_x_cond
        while num_generated <= num_samples:

            print(num_generated)
            if num_generated == 0:
                log_x_cond_batch = log_x_cond[:batch_size]
            else:
                log_x_cond_batch = log_x_cond[num_generated:num_generated+batch_size]

            if log_x_cond_batch.shape[0] < b:
                missing_rows = b - log_x_cond_batch.shape[0]

                padding = torch.zeros(missing_rows, log_x_cond_batch.shape[1])

                log_x_cond_batch = torch.cat(
                    [log_x_cond_batch, padding], dim=0)

            if not ddim:
                sample = sample_fn(b, log_x_cond=log_x_cond_batch)
            else:
                sample = sample_fn(b, steps=steps, log_x_cond=log_x_cond_batch)
