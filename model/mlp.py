...
def _get_labels_and_probs(
    ...
    labels = np.round(
        probs) if task_type == TaskType.BINCLASS else probs.argmax(axis=1)
...
def calculate_metrics_(
    ...
    if task_type == TaskType.REGRESSION:
        ...
        labels, probs = _get_labels_and_probs(
            y_pred, task_type, prediction_type)
        result = cast(
            Dict[str, Any], skm.classification_report(
                y_true, labels, output_dict=True)
        )
        if task_type == TaskType.BINCLASS:
...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in,  num_classes, is_y_cond, rtdl_params, dim_t=1024):
        ...
        self.proj = nn.Linear(108, dim_t)
        self.proj_cond = self.time_embed = nn.Sequential(
            nn.Linear(2, dim_t),
            nn.SiLU(),
    ...
    def forward(self, x_non_cond, x_cond, timesteps, y=None):
        ...
        # print(x_cond.shape, len(np.unique(x_cond.to('cpu'))))
        # print(F.silu(self.label_emb(x_cond.squeeze().long())).shape)
        # print(F.silu(self.label_emb(x_cond.long())))
        emb += F.silu(self.label_emb(x_cond.squeeze().long()))
        # x = self.proj(x_non_cond) + F.silu(self.proj_cond(x_cond)) + emb
        x = self.proj(x_non_cond) + emb
